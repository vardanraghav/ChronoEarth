          ...
          <div className="lg:col-span-1 flex flex-col gap-6">
            <div className="card-tier-1 flex flex-col gap-6">
              ...
              <div className="flex flex-col items-center gap-4 text-center">
                {avatarError ? (
                  <div className="w-28 h-28 rounded-full border-2 border-[#00F5B0] flex items-center justify-center bg-[#040B12] text-xl font-mono text-[#00F5B0] font-bold shadow-[0_0_20px_rgba(0,245,176,0.15)] shrink-0">
                  ...
                  <img 
                    ...
                    className="w-28 h-28 rounded-full border-2 border-[#00F5B0] object-cover shadow-none shrink-0"
                  />
                )}
                <div>
                  <h1 className="text-2xl font-light text-white">{f.name}</h1>
                  <span className="text-xs text-[#00F5B0] block mt-1">{f.role}</span>
                </div>
              </div>

              <div className="border-t border-[#00F5B0]/15 pt-4 flex flex-col gap-3 text-xs">
                <div>
                  <span className="text-[#7A8694] block">Specialization area</span>
                  <span className="text-[#00D98F]">{f.specialization}</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1">
                    ...
                    <span className="text-[#00F5B0] font-mono">{f.influenceScore}%</span>
                  </div>
                </div>
              </div>

              <div className="border-t border-[#00F5B0]/15 pt-4 text-xs text-[#7A8694] leading-relaxed">
            ...
            <div className="card-tier-2 h-full flex flex-col gap-6">
              ...
              <div className="flex items-center gap-3 border-b border-[#00F5B0]/15 pb-4">
                <h2 className="text-lg font-light text-white">
                  Active forecasts by <span className="text-[#00F5B0]">{f.name}</span>
                </h2>
              </div>
                    ...
                    >
                      <div className="flex justify-between items-center text-xs">
                        <div className="flex gap-2">
                          <span className="px-2 py-0.5 bg-[#040B12] text-[#00F5B0] border border-[#00F5B0]/15 rounded-sm">{p.category}</span>
                          <span className="px-2 py-0.5 bg-[#040B12] text-[#00F5B0] border border-emerald-500/15 rounded-sm">{p.year} forecast</span>
                        </div>
                      ...
                      <div className="flex justify-between items-center border-t border-[#00F5B0]/15 pt-3 text-xs text-[#7A8694]">
                        <span>Location: {p.city}</span>
                        <Link 
                          href={`/predictions/${p.slug}`}
                          className="text-[#00F5B0] hover:underline"
                        >
                          Analyze forecast →
                        </Link>
                      </div>
