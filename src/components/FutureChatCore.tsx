...
export default function FutureChatCore() {
  ...
  // Scroll to bottom only if user is already near bottom or sent a message
  useEffect(() => {
  ...
  // Adjust scroll when typing state changes
  useEffect(() => {
    if (typing) {
      const container = messagesContainerRef.current;
      if (container) {
  ...
  // Scroll to bottom when AI response finishes
  useEffect(() => {
  ...
  const handleSend = async (e?: React.FormEvent, customPrompt?: string) => {
    ...
    // Save user message to database asynchronously if user is logged in
    if (user) {
      supabase.from('futurechat_conversations').insert({
        user_id: user.id,
        role: 'user',
        message: prompt
    ...
    try {
      const res = await fetch('/api/futurechat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: prompt }) });
      const data = await res.json();
      const answer = data?.success ? (data.answer || 'No answer') : (`ChronoAI error: ${data?.error || 'unknown'}`);
      const aiMsg: ChatMessage = { id: `a-${Date.now()}`, text: answer, isAi: true, avatar: 'https://images.unsplash.com/photo-1620712943543-bcc4688e7485?w=80&auto=format&fit=crop&q=80' };
      setMessages(prev => [...prev, aiMsg]);

      // Save assistant message to database asynchronously if user is logged in
      if (user) {
        supabase.from('futurechat_conversations').insert({
          user_id: user.id,
          role: 'assistant',
          ...
          if (error) console.error('Failed to persist AI response:', error.message);
        });
      }
    } catch (err) {
      ...
      if (user) {
        supabase.from('futurechat_conversations').insert({
          ...
          if (error) console.error('Failed to persist AI error response:', error.message);
        });
      }
    } finally {
      setTyping(false);
  ...
  return (
    <main className="h-screen w-screen bg-[#02060A] text-[#E2E8F0] relative overflow-hidden flex flex-col font-sans">
      ...
      <div className="flex-1 pt-24 pb-4 px-6 flex flex-col items-center relative z-10 min-h-0">
        <div className="w-full max-w-3xl flex flex-col h-full gap-4 min-h-0">
          ...
          <header className="mb-2 shrink-0">
            ...
            <p className="text-sm text-[#94A3B8] font-mono">AI-powered future intelligence assistant</p>
          </header>
          ...
          <div className="flex-1 bg-[#040B12]/80 rounded-lg p-4 flex flex-col min-h-0 border border-white/5 relative">
                ...
                <div className="w-full h-full flex flex-col items-center justify-center text-center text-[#94A3B8] gap-6 p-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-xl mt-4">
                    {suggestions.map((s, idx) => (
                      ...
                      >
                        <span className="font-bold text-[10px] uppercase tracking-wider text-slate-400 group-hover:text-[#00F5B0]">{s.label}</span>
            ...
            <form onSubmit={(e) => { handleSend(e); }} className="mt-3 flex items-center gap-2 shrink-0 border-t border-white/5 pt-3">
              <span className="text-[#00F5B0] font-bold font-mono text-xs select-none pl-2">chrono_os:~$ &gt;</span>
              <input
                type="text"
                ...
                className="flex-1 bg-transparent border-none outline-none text-xs text-white placeholder-white/20 font-mono disabled:opacity-50"
              />
              <button 
                type="submit" 
                disabled={isGenerating || !inputValue.trim()} 
                className="px-4 py-2 bg-[#00F5B0] hover:bg-[#00D98F] text-black font-bold font-mono text-xs rounded uppercase tracking-wider transition-all cursor-pointer shadow-[0_0_10px_rgba(0,245,176,0.3)] disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Transmit
              </button>
            </form>
