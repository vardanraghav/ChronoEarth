...
async function run() {
  ...
  try {
    ...
    console.log('Navigating to http://localhost:3000 (Cyber Mode by default)...');
    await page.goto('http://localhost:3000', { waitUntil: 'networkidle2', timeout: 40000 });
    console.log('Waiting 15 seconds for Cesium Globe and Cyber HUD...');
    ...
    const cityUrl = 'http://localhost:3000/city/tokyo';
    ...
    // 4. Predictions Page
    const predUrl = 'http://localhost:3000/predictions';
    console.log(`Navigating to ${predUrl}...`);
    await page.goto(predUrl, { waitUntil: 'networkidle2', timeout: 30000 });
    ...
    await page.goto(feedbackUrl, { waitUntil: 'networkidle2', timeout: 30000 });
    console.log('Waiting 5 seconds...');
    await new Promise(resolve => setTimeout(resolve, 5000));
    ...
    console.error('Error occurred during capturing:', err);
  } finally {
    await browser.close();
